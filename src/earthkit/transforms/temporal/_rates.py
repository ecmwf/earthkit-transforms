...
def _accumulation_to_rate_dataarray_take2(
    dataarray: xr.DataArray,
    step: None | float | int = None,
    step_units: str = "hours",
    rate_units: str = "seconds",
...
def _start_of_forecast_accumulation_to_rate(
    dataarray, time_dim_array, step_dim_array, step_float, time_dim, from_first_step, rate_scale_factor, xp
) -> xr.DataArray:
    # Compute forward differences
    diff_data = dataarray.diff(time_dim, label="upper")
    # If step_dim_array is provided, use it to compute time differences
    if step_dim_array is not None:
        time_diff = step_dim_array.diff(time_dim, label="upper")
    else:
        time_diff = time_dim_array.diff(time_dim, label="upper")
    mask = xr.apply_ufunc(
        xp.isclose,
